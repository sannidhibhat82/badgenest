...
import { Award, CheckCircle, XCircle, AlertTriangle, Calendar, Shield, Eye, Trophy, ExternalLink } from "lucide-react";
...
export default function PublicProfile() {
  ...
  const initials = data?.profile?.full_name
    ...
    : "?";
  const avatarUrl = data?.profile?.avatar_url?.startsWith("http") ? data.profile.avatar_url : data?.profile?.avatar_url ? `${API_BASE}${data.profile.avatar_url}` : null;
  const uniqueIssuerCount = data ? new Set(data.assertions.map((a: any) => a.badge?.issuer_id).filter(Boolean)).size : 0;
  ...
  const activeCount = data.assertions.filter(
    (a: any) => !a.revoked && !(a.expires_at && new Date(a.expires_at) < new Date())
  ).length;
  ...
  return (
    <>
      <Helmet>
        ...
        <meta name="description" content={`View ${data.profile.full_name}'s ${activeCount} earned digital badges on BadgeNest.`} />
      </Helmet>
              ...
              <div className="text-center sm:text-left flex-1">
                ...
                <div className="mt-4 flex flex-wrap items-center justify-center sm:justify-start gap-4">
                  ...
                  <div className="flex items-center gap-1.5 rounded-full bg-card/80 backdrop-blur-sm px-3 py-1.5 text-sm shadow-sm border border-border/50">
                    ...
                    <span className="font-semibold text-foreground">{uniqueIssuerCount}</span>
                    <span className="text-muted-foreground">issuer{uniqueIssuerCount !== 1 ? "s" : ""}</span>
                  </div>
                  {data.totalViews > 0 && (
                    <div className="flex items-center gap-1.5 rounded-full bg-card/80 backdrop-blur-sm px-3 py-1.5 text-sm shadow-sm border border-border/50">
        ...
        <main className="container mx-auto max-w-4xl px-4 py-8">
          {data.assertions.length === 0 ? (
            <div className="flex flex-col items-center justify-center rounded-2xl border border-dashed border-border/60 p-16 text-center animate-fade-in">
              ...
              <p className="mt-1 text-sm text-muted-foreground">Badges will appear here once earned.</p>
            </div>
          ) : (
            <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3 stagger-children">
              {data.assertions.map((a: any) => {
                const status = getStatus(a);
                const StatusIcon = status.icon;
                const badgeImageUrl = a.badge?.image_url?.startsWith("http") ? a.badge.image_url : a.badge?.image_url ? `${API_BASE}${a.badge.image_url}` : null;
                return (
                        ...
                        <div className="flex items-start gap-4">
                          {badgeImageUrl ? (
                            <div className="shrink-0 rounded-xl bg-muted/50 p-1.5 ring-1 ring-border group-hover:ring-primary/20 transition-all duration-300">
                              <img src={badgeImageUrl} alt={a.badge?.name} className="h-14 w-14 rounded-lg object-contain transition-transform duration-300 group-hover:scale-110" />
                            </div>
                          ) : (
                          ...
                          <div className="flex-1 min-w-0">
                            <p className="font-semibold text-foreground truncate group-hover:text-primary transition-colors">{a.badge?.name ?? "Unknown Badge"}</p>
                            <p className="text-xs text-muted-foreground truncate mt-0.5">{a.badge?.issuer?.name ?? "Unknown Issuer"}</p>
                            <div className="mt-2">

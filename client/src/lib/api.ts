...
export const auth = {
  ...
  signup: (email: string, password: string, full_name?: string) =>
    api<{
      message: string;
      token?: string;
      user: { id: string; email: string; full_name: string | null; avatar_url: string | null; email_verified: boolean };
    }>("/api/auth/signup", { method: "POST", body: { email, password, full_name } }),
  session: () =>
  ...
  resendVerification: (email: string) =>
    api<{ message: string }>("/api/auth/resend-verification", { method: "POST", body: { email } }),

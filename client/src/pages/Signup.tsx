...
export default function Signup() {
  ...
  const handleSignup = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      const result = await auth.signup(email, password, fullName || undefined);
      if (result.token) {
        localStorage.setItem("token", result.token);
        await refreshSession();
        toast({ title: "Account created!", description: result.message || "You're signed in." });
        navigate(redirect || "/dashboard");
      } else {
        toast({ title: "Account created!", description: result.message || "Please verify your email before signing in." });
